...
class Structures_BibTex
{
    ...
    function setOption($option, $value)
    {
        ...
        if (array_key_exists($option, $this->_options)) {
            ...
            //$ret = raiseError('Unknown option '.$option);
        //}
        return $ret;
    }
    ...
    function loadFile($filename)
    {
        if (file_exists($filename)) {
            if (($this->content = @file_get_contents($filename)) === false) {
                //return raiseError('Could not open file '.$filename);
		...
		return false;
            ...
            //return raiseError('Could not find file '.$filename);
	    return false;
        }
    }
    ...
    function parse()
    {
            ...
            if ((0 == $open) && ('@' == $char)) { //The beginning of an entry
                ...
                if (0 == $open) { //End of entry
                    ...
                    if (!$entrydata) {
                        ...
                        $this->data[$entrydata['cite']] = $entrydata;
                    }
                    $buffer = '';
                }
            }
        ...
        if ($valid) {
            ...
            return raiseError('Unbalanced parenthesis');
        }
    }
    ...
    function _determineCase($word) {
        ...
        if (is_string($word) && (strlen($trimmedword) > 0)) {
            ...
            $ret = raiseError('Could not determine case on word: '.(string)$word);
        }
        return $ret;
    }
     ...
     */
    function html($entries = 0)
    {
	    $month_array = array( "jan" => 1, "feb" =>2, "mar" =>3, "apr" =>4, 
	    ...
	    $selected = array();
	    if ($entries!=0) {
		    foreach($entries as $e) $selected[$e] = true;
	    }
		...
		if ($entries!=0 && !$selected[$key]) {
			continue;
		}
		$selection[$key] = $entry;
		$priority = (int)$this->allowedEntryTypes[$entry['entryType']] * 1e6;
    		if (array_key_exists('year', $entry)) {
    			$priority += (int) (100 * ($this->_unwrap($entry['year'])));
			if (array_key_exists('month', $entry)) {
				// TODO: convert month in number.
				$m = strtolower($this->_unwrap($entry['month']));
				$priority += $month_array[$m[0].$m[1].$m[2]];
		...
		//print $entry['cite']." -> $priority, ".
		//	$entry['month']." <br/>";
		//$this->data[$key]['intdate'] = $priority;
		$selection[$key]['intdate'] = $priority;
    	}

	usort($selection, "my_cmp_date");

	$prev_type = false;
	$prev_date = false;
        $date_ul_opened = false;
	    ...
	    if (array_key_exists('booktitle', $entry)) {
		    $journal = $this->_unwrap($entry['booktitle']);
	    }
	    ...
	    if ($journal) {
	    $others = array("month" => ", ", "year" => " " , "address" => ", ", "volume" => ", volume ", "number" => ", number ");
	    foreach ($others as $tag => $sep) {
		    if (array_key_exists($tag, $entry)) {
			$journal .= $sep . $this->_unwrap($entry[$tag]);
		    }
	    }
	    }
            if ((''!=$title) || (''!=$journal) || (''!=$year) || (''!=$authors)) {
		...
		if ($this->html_hierarchy == "full") {
			if ($type != $prev_type) {
				...
				if ($count_by_type[$type]>1) {
					$line = "    <li>$year\n      <ul class=\"publi-entry\">\n      ".$line;
					$prev_date=$year;
                                        $date_ul_opened = true;
				}
	...
	if ($prev_date) $ret = $ret."    </ul>\n    </li>\n";
	if ($prev_type) $ret = $ret."  </ul>\n  </li>\n";
        return "<ul class=\"publi-type\">\n" . $ret . "</ul>\n";
    }
}

    function my_cmp_date($a, $b) {
	    $ia = (array_key_exists('intdate', $a) ? $a['intdate'] : 12*3000);
	    $ib = (array_key_exists('intdate', $b) ? $b['intdate'] : 12*3000);
	    if ($ia == $ib) $r = 0;
	    else $r = ($ia>$ib ? -1 : 1); 
	    return $r;
    }
